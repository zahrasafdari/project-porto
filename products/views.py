...
def setting_page(request):
	account_form = AccountForm(request.POST or None, request.FILES or None, instance=request.user)

	active_template = request.user.active_template
	if active_template:
		template_form = template_map[active_template.name]['form']
		template_obj = template_map[active_template.name]['class_obj'].objects.get(user=request.user)

		template_form = template_form(request.POST or None, request.FILES or None, instance=template_obj)
